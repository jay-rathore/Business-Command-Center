...
export class GoogleAnalyticsSyncService {
  ...
  async scheduledSync(): Promise<void> {
    const active = await this.connections.listActive(IntegrationProvider.GOOGLE_ANALYTICS);
    for (const connection of active) {
      await TenantContext.run({ organizationId: connection.organizationId }, async () => {
        try {
          const credentials = this.connections.decrypt<GoogleAnalyticsCredentials>(connection);
  ...
  async syncNow(credentials: GoogleAnalyticsCredentials): Promise<GoogleAnalyticsSyncResult> {
  ...
  private getClient(credentials: GoogleAnalyticsCredentials): BetaAnalyticsDataClient {
